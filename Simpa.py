...
def vector_to_parameters(
    vec: torch.Tensor, parameter_shapes: typing.List[torch.Size]
...
class FunctionalGenerator(torch.nn.Module):
    ...
    def forward_vector(
        self, z: torch.Tensor, w: typing.List[torch.Tensor]
    ) -> torch.Tensor:
        ...
        """
        param_vec = torch.nn.functional.linear(input=z, weight=w[0], bias=w[1])
        ...
        param_vec = torch.nn.functional.linear(input=param_vec, weight=w[2], bias=w[3])
        param_vec = torch.nn.functional.relu(input=param_vec)
        # param_vec = torch.nn.functional.leaky_relu(input=param_vec, negative_slope=0.1)
        # param_vec = torch.nn.functional.dropout(input=param_vec, p=0.25, training=train)

        param_vec = torch.nn.functional.linear(input=param_vec, weight=w[4], bias=w[5])
    ...
    def forward(self, z: torch.Tensor, w: typing.List[torch.Tensor]) -> torch.Tensor:
        ...
        params = vector_to_parameters(
            vec=param_vec[0], parameter_shapes=self.param_shapes
        )
...
class Simpa(MLBaseClass):
    ...
    def __init__(self, config: dict) -> None:
        ...
        config["p_w"] = torch.distributions.normal.Normal(
            loc=torch.tensor(0.0, device=config["device"]),
            scale=torch.tensor(1.0, device=config["device"]),
        )
    ...
    def load_model(
        self, resume_epoch: int, eps_dataloader: torch.utils.data.DataLoader
    ) -> dict:
        ...
        if self.config["network_architecture"] == "FcNet":
            base_net = FcNet(
                dim_output=self.config["num_ways"], num_hidden_units=(40, 40)
            )
        ...
        print(
            "Number of parameters of the base network = {0:,}.\n".format(
                self.base_net_num_params
            )
        )
        ...
        model["hyper_net"] = IdentityNet(
            base_net=torch.nn.Sequential(
                ...
                torch.nn.ReLU(),
                torch.nn.Linear(in_features=512, out_features=self.base_net_num_params),
            )
        )
        ...
        phi_base_net = torch.nn.Sequential(
            torch.nn.Linear(in_features=self.base_net_num_params, out_features=512),
        ...
        model["phi_optimizer"] = torch.optim.Adam(
            params=model["phi_hyper_net"].parameters(), lr=self.config["phi_lr"]
        )
        # endregion
        ...
        if resume_epoch > 0:
            ...
            saved_checkpoint = torch.load(
                ...
                map_location=lambda storage, loc: (
                    storage.cuda(device=self.config["device"].index)
    ...
    def train(
        ...
        model = self.load_model(
            resume_epoch=self.config["resume_epoch"], eps_dataloader=train_dataloader
        )
        ...
        try:
            for epoch_id in range(
                ...
                for eps_count, eps_data in enumerate(train_dataloader):
                    ...
                    KL_loss = (
                        KL_lower_bound + np.log(y_v.numel()) / self.config["epsilon"]
                    ) / (2 * (y_v.numel() - 1))
                    ...
                    if (eps_count + 1) % self.config["minibatch"] == 0:
                        ...
                        torch.nn.utils.clip_grad.clip_grad_norm_(
                            parameters=model["hyper_net"].parameters(), max_norm=10
                        )
                        ...
                        torch.nn.utils.clip_grad.clip_grad_norm_(
                            parameters=model["phi_hyper_net"].parameters(), max_norm=10
                        )
                        ...
                        if (eps_count + 1) % self.config["minibatch_print"] == 0:
                            ...
                            global_step = (
                                epoch_id * self.config["num_episodes_per_epoch"]
                                + eps_count
                                + 1
                            ) // self.config["minibatch_print"]
                ...
                print(
                    "State dictionaries are saved into {0:s}\n".format(checkpoint_path)
                )
    ...
    def adaptation(
        self, x: torch.Tensor, y: torch.Tensor, model: dict
        ...
        for _ in range(self.config["num_inner_updates"]):
            ...
            for _ in range(self.config["num_models"]):
                ...
                if self.config["first_order"]:
                    grads = torch.autograd.grad(
                        outputs=cls_loss, inputs=generator_params, retain_graph=True
                    )
                else:
                    grads = torch.autograd.grad(
                        outputs=cls_loss, inputs=generator_params, create_graph=True
                    )
                ...
                for i in range(len(grads)):
                    ...
                    grads_accum[i] = (
                        grads_accum[i] + grads[i] / self.config["num_models"]
                    )

            # loss related to KL[q(w; lambda) || p(w)]
            new_phi_params = self.train_phi(
                generator_params=generator_params, phi_params=phi_params, model=model
            )
            phi_params = [new_phi_param + 0.0 for new_phi_param in new_phi_params]
            KL_lower_bound = self.estimate_KL_lower_bound(
                generator_params=generator_params, phi_params=phi_params, model=model
            )
            KL_loss = (KL_lower_bound + np.log(y.numel()) / self.config["epsilon"]) / (
                2 * (y.numel() - 1)
            )
            KL_loss = torch.sqrt(input=KL_loss)
            if torch.isnan(input=KL_loss) or (KL_loss.item() < 0):
                ...
                KL_grads = torch.autograd.grad(
                    outputs=KL_loss, inputs=generator_params, retain_graph=True
                )
                # accumulate gradients from Monte Carlo sampling and average out
            ...
            for i in range(len(generator_params)):
                new_generator_params[i] = (
                    generator_params[i] - self.config["inner_lr"] * grads_accum[i]
                )

            generator_params = [
                new_generator_param + 0 for new_generator_param in new_generator_params
            ]
    ...
    def validation_loss(
        ...
        logits = self.prediction(
            x=x, adapted_generator_params=adapted_generator_params, model=model
        )
    ...
    def evaluation(
        ...
        logits = self.prediction(
            x=x_v, adapted_generator_params=adapted_generator_params, model=model
        )

        cls_loss = self.validation_loss(
            x=x_v, y=y_v, adapted_generator_params=adapted_generator_params, model=model
        )
    ...
    def estimate_KL_lower_bound(
        ...
        KL_lower_bound = torch.mean(
            input=model["f_phi_base_net"].forward(param_vecs, params=phi_params)
        )

        # generate parameters from prior p
        param_vecs = self.config["p_w"].sample(
            sample_shape=(num_samples, self.base_net_num_params)
        )
        KL_lower_bound = KL_lower_bound - torch.logsumexp(
            input=model["f_phi_base_net"].forward(param_vecs, params=phi_params),
            dim=(0, 1),
        )
    ...
    def train_phi(
        ...
        for _ in range(1):
            ...
            KL_grads = torch.autograd.grad(
                outputs=KL_lower_bound, inputs=phi_params, retain_graph=True
            )

            new_phi_params = [None] * len(phi_params)
            for i in range(len(phi_params)):
                new_phi_params[i] = (
                    phi_params[i] + self.config["phi_inner_lr"] * KL_grads[i]
                )
                # with torch.no_grad():
    ...
    def KL_divergence_standard_normal(
        p: typing.List[torch.Tensor],
    ) -> typing.Union[torch.Tensor, float]:
