...
config["load_images"] = (
    True  # load images on RAM for fast access. Set False for large dataset to avoid out-of-memory
)
...
config["num_models"] = (
    1  # number of models used in Monte Carlo to approximate expectation
)
...
config["network_architecture"] = (
    "CNN"  # either CNN or ResNet18 specified in the CommonModels.py
)
...
config["episode_file"] = (
    None  # path to a csv file with row as episode name and column as list of classes that form an episode
)
...
class MLBaseClass(object):
    ...
    ) -> typing.Union[higher.patch._MonkeyPatchBase, torch.Tensor]:
    ...
    def prediction(
        self,
        x: torch.Tensor,
        adapted_hyper_net: typing.Union[torch.Tensor, higher.patch._MonkeyPatchBase],
        model: dict,
    ...
    def test(self, num_eps: int, eps_dataloader: torch.utils.data.DataLoader) -> None:
        ...
        print(
            "NLL = {0} +/- {1}".format(
                np.mean(loss), 1.96 * np.std(loss) / np.sqrt(len(loss))
            )
        )
        print(
            "Accuracy = {0:.2f} +/- {1:.2f}\n".format(
                np.mean(accuracy), 1.96 * np.std(accuracy) / np.sqrt(len(accuracy))
            )
        )
        return None