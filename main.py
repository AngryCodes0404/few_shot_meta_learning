...
parser.add_argument(
    ...
    type=str,
    default="../datasets",
    help="Parent folder containing the dataset",
)
parser.add_argument(
    "--datasource",
    action="append",
    help="List of datasets: SineLine for regression, and omniglot, miniImageNet, ImageNet for classification",
)

parser.add_argument(
    "--img-size", action="append", help="A pair of image size: 32 or 84"
)
...
parser.add_argument(
    ...
    type=str,
    default="MAML",
    help="Few-shot learning methods, including: MAML, vampire or protonet",
)
...
parser.add_argument(
    ...
    type=float,
    default=1e-6,
    help="Weighting factor for the KL divergence (only applicable for VAMPIRE)",
)
...
parser.add_argument(
    ...
    type=str,
    default="CNN",
    help="The base model used, including CNN and ResNet18 defined in CommonModels",
)
...
parser.add_argument(
    "--num-ways", type=int, default=5, help="Number of classes within a task"
)
...
parser.add_argument(
    ...
    type=float,
    default=0.01,
    help="Learning rate of episode adaptation step",
)

parser.add_argument(
    "--logdir",
    type=str,
    default="/media/n10/Data/",
    help="Folder to store model and logs",
)

parser.add_argument(
    "--meta-lr", type=float, default=1e-3, help="Learning rate for meta-update"
)
parser.add_argument(
    "--minibatch",
    type=int,
...
parser.add_argument(
    "--k-shot", type=int, default=1, help="Number of training examples per class"
)
parser.add_argument(
    "--v-shot", type=int, default=15, help="Number of validation examples per class"
)

parser.add_argument(
    "--num-episodes-per-epoch",
    type=int,
    default=10000,
...
parser.add_argument(
    "--num-workers", type=int, default=2, help="Number of workers used in data loader"
)
...
parser.add_argument(
    ...
    type=int,
    default=1,
    help="Number of base network sampled from the hyper-net",
)

parser.add_argument(
    "--num-episodes", type=int, default=100, help="Number of episodes used in testing"
)
...
if __name__ == "__main__":
    if "SineLine" not in config["datasource"]:
        ...
        train_dataloader = torch.utils.data.DataLoader(
            dataset=regression_dataset, shuffle=True
        )
        test_dataloader = torch.utils.data.DataLoader(
            dataset=regression_dataset, shuffle=True
        )
