...
class EpisodeSampler(torch.utils.data.BatchSampler):
    ...
    def __init__(
        self,
        sampler: torch.utils.data.Sampler[typing.List[int]],
        num_ways: int,
    ...
    def __iter__(self) -> typing.Iterator[typing.List[int]]:
        while True:
            ...
            dataset_id = random.randint(
                a=0, b=len(self.sampler.data_source.datasets) - 1
            )

            # n-way
            labels = random.sample(
                population=self.class_img_idx[dataset_id].keys(), k=self.num_ways
            )
            ...
            for label in labels:
                batch.extend(
                    random.sample(
                        population=self.class_img_idx[dataset_id][label],
