...
class IdentityNet(torch.nn.Module):
    ...
    def __init__(self, base_net: torch.nn.Module, **kwargs) -> None:
        ...
        self.params = torch.nn.ParameterList(
            [torch.nn.Parameter(p.float()) for p in params]
        )
...
class NormalVariationalNet(torch.nn.Module):
    ...
    def __init__(self, base_net: torch.nn.Module, **kwargs) -> None:
        ...
        self.log_std = torch.nn.ParameterList(
            [
                torch.nn.Parameter(torch.rand_like(v) - 4)
...
class EnsembleNet(torch.nn.Module):
    ...
    def __init__(self, base_net: torch.nn.Module, **kwargs) -> None:
        ...
        for _ in range(self.num_particles):
            params_list = intialize_parameters(
                state_dict=base_state_dict
            )  # list of tensors
            params_vec = torch.nn.utils.parameters_to_vector(
    ...
    def forward(self, i: int) -> typing.List[torch.Tensor]:
        return vector_to_list_parameters(
            vec=self.params[i], parameter_shapes=self.parameter_shapes
        )
...
class PlatipusNet(torch.nn.Module):
    ...
    def __init__(self, base_net: torch.nn.Module, **kwargs) -> None:
        ...
        self.params.append(
            parameter=torch.nn.Parameter(data=torch.randn(size=(self.num_base_params,)))
        )
        self.params.append(
            parameter=torch.nn.Parameter(
                data=torch.randn(size=(self.num_base_params,)) - 4
            )
        )
        self.params.append(
            parameter=torch.nn.Parameter(
        ...
        self.params.append(
            parameter=torch.nn.Parameter(data=torch.tensor(0.01))
        )  # gamma_p
        self.params.append(
            parameter=torch.nn.Parameter(data=torch.tensor(0.01))
        )  # gamma_q
    ...
    def forward(self) -> dict:
        ...
        meta_params = dict.fromkeys(
            ("mu_theta", "log_sigma_theta", "log_v_q", "gamma_p", "gamma_q")
        )

        meta_params["mu_theta"] = vector_to_list_parameters(
            vec=self.params[0], parameter_shapes=self.parameter_shapes
        )
        meta_params["log_sigma_theta"] = vector_to_list_parameters(
            vec=self.params[1], parameter_shapes=self.parameter_shapes
        )
        meta_params["log_v_q"] = vector_to_list_parameters(
            vec=self.params[2], parameter_shapes=self.parameter_shapes
        )
