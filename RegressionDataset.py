...
class SineDataset(torch.utils.data.Dataset):
    ...
    def generate_label(self) -> torch.Tensor:
        ...
        amplitude = (
            torch.rand(size=(1,)) * (self.amplitude_range[1] - self.amplitude_range[0])
            + self.amplitude_range[0]
        )
        phase = (
            torch.rand(size=(1,)) * (self.phase_range[1] - self.phase_range[0])
            + self.phase_range[0]
        )
...
class LineDataset(torch.utils.data.Dataset):
    ...
    def __init__(
        self,
        slope_range: typing.Tuple[float],
        intercept_range: typing.Tuple[float],
        x_range: typing.Tuple[float],
        num_samples: int,
        noise_std: float,
    ) -> None:
    ...
    def generate_label(self) -> torch.Tensor:
        slope = (
            torch.rand(size=(1,)) * (self.slope_range[1] - self.slope_range[0])
            + self.slope_range[0]
        )
        intercept = (
            torch.rand(size=(1,)) * (self.intercept_range[1] - self.intercept_range[0])
            + self.intercept_range[0]
        )
