...
class Platipus(object):
    ...
    def load_model(
        self, resume_epoch: int, eps_dataloader: torch.utils.data.DataLoader, **kwargs
    ) -> dict:
        maml_temp = Maml(config=self.config)
        return maml_temp.load_model(
            resume_epoch=resume_epoch, eps_dataloader=eps_dataloader, **kwargs
        )
    ...
    def adapt_params(
        ...
        q_params = [p + 0.0 for p in params]
        ...
        for _ in range(self.config["num_inner_updates"]):
            ...
            if self.config["first_order"]:
                grads = torch.autograd.grad(
                    outputs=loss, inputs=q_params, retain_graph=True
                )
            else:
                grads = torch.autograd.grad(
                    outputs=loss, inputs=q_params, create_graph=True
                )
    ...
    def adaptation(
        ...
        mu_theta_t = self.adapt_params(
            x=x,
            y=y,
            params=params_dict["mu_theta"],
            lr=params_dict["gamma_p"],
            model=model,
        )
        ...
        for model_id in range(self.config["num_models"]):
            ...
            for i in range(len(theta)):
                theta[i] = mu_theta_t[i] + torch.randn_like(
                    input=mu_theta_t[i], device=mu_theta_t[i].device
                ) * torch.exp(input=params_dict["log_sigma_theta"][i])
    ...
    def prediction(
        self, x: torch.Tensor, phi: typing.List[typing.List[torch.Tensor]], model: dict
    ) -> typing.List[torch.Tensor]:
    ...
    def validation_loss(
        ...
        for model_id in range(self.config["num_models"]):
            ...
            theta = [None] * len(params_dict["mu_theta"])
            for i in range(len(theta)):
                theta[i] = mu_theta_v[i] + torch.randn_like(
                    input=mu_theta_v[i], device=mu_theta_v[i].device
                ) * torch.exp(input=params_dict["log_v_q"][i])
        ...
        KL_loss = kl_divergence_gaussians(
            p=[*mu_theta_v, *params_dict["log_v_q"]],
            q=[*mu_theta_t, *params_dict["log_sigma_theta"]],
        )
    ...
    def evaluate(
        self, num_eps: int, eps_dataloader: torch.utils.data.DataLoader, model: dict
